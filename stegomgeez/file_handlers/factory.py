...
from file_handlers.image_file import ImageFile, JPGFile, PNGFile
...
class AsyncFileFactory:
    ...
    @staticmethod
    async def _parse_image(file_path: Path, data: bytes, mime: str) -> ImageFile:
        raw_array: ndarray = frombuffer(data, uint8)
        cvimg: ndarray = cv2.imdecode(raw_array, cv2.IMREAD_UNCHANGED)
        if mime == 'image/jpeg':
            return JPGFile(file_path, cvimg, mime)
        elif mime == 'image/png':
            return PNGFile(file_path, cvimg, mime)

    @staticmethod
    async def _parse_text(file_path: Path, data: str,):
        pass

    @staticmethod
    async def _parse_binary(file_path: Path, data: bytes):
        pass

    @classmethod
    async def _parse_file_type(cls, file_path: Path, data: bytes, ftype: str, mime: str) -> FileType:
    ...
    @classmethod
    async def create_from_bytes(cls, name: str, output_dir: Path, data: bytes) -> FileType:
    ...
    @classmethod
    async def create_from_path(cls, file_path: Path) -> FileType:
